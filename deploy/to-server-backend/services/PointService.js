...
/**
 * 获取等级信息（level 可能来自 DB 的字符串，需归一化）
 */
function getLevelInfo(level) {
    const n = Number(level);
    const idx = Number.isFinite(n) ? n : 1;
    return LEVEL_CONFIG.find(c => c.level === idx) || LEVEL_CONFIG[0];
}
...
async function getAccountInfo(userId) {
    ...
    const nextLevelInfo = LEVEL_CONFIG.find(c => c.level === derivedLevel + 1);
    ...
    return {
        ...
        next_level: nextLevelInfo ? {
            ...
            points_needed: Math.max(0, nextLevelInfo.min - totalPoints)
        } : null,
        is_free_shipping: derivedLevel >= 1  // Lv1+ 全场包邮
    };
}
