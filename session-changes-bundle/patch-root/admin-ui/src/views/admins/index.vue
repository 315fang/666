...
const defaultRolePerms = {
  admin: [
    ...
    'dealers', 'commissions', 'statistics', 'logs', 'settings_manage', 'notification',
...
const openAdminForm = (row) => {
  if (row) {
    ...
    Object.assign(adminForm, {
      id: null, username: '', password: '', name: '', phone: '', email: '',
      role: 'operator', permissions: [...(defaultRolePerms['operator'] || [])], status: 1
    })
  }
...
const onRoleChange = (role) => {
  if (defaultRolePerms[role]) {
    adminForm.permissions = [...defaultRolePerms[role]]
  }
}
