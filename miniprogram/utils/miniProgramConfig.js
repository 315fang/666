...
/** 按 index 与默认项对齐文案与颜色，避免后台只存部分项时整表替换导致异常 */
...
function getMiniProgramConfig() {
  const app = getApp();
  const config = app?.globalData?.miniProgramConfig || {};
...
function getFeatureFlags() {
  return getConfigSection('feature_flags');
}

function getFeatureToggles() {
  return getMiniProgramConfig().feature_toggles || {};
}

function getLightPromptModals() {
