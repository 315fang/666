        ...
        try {
            ...
            for (const item of items) {
                ...
                if (sku_id) {
                    ...
                    price = PricingService.calculateDisplayPrice(product, sku, roleLevel, purchaseLevel);
                    stockTarget = sku;
                } else {
                    price = PricingService.calculateDisplayPrice(product, null, roleLevel, purchaseLevel);
                }
                // 砍价单：用砍价价格覆盖，不再叠加折扣
                ...
                if (slash_no) {
                    ...
                    // 复购折扣：爆品/折扣商品（discount_exempt=true）不参与
                    if (finalDiscountRate > 0 && finalDiscountRate < 1 && !product.discount_exempt) {
                        price = parseFloat((price * finalDiscountRate).toFixed(2));
                    }
                }
