...
function resolveCommissionBalance(user = {}) {
    return toNumber(user.commission_balance ?? user.balance ?? user._commission_balance, 0);
}

function resolveGoodsFundBalance(user = {}) {
    return toNumber(user.agent_wallet_balance ?? user.wallet_balance ?? user._agent_wallet_balance, 0);
}
