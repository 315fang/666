...
// 从 product-bundles/index.vue 拆出"自由选套餐编辑抽屉"，承担：
//   - 抽屉容器 + 套餐基础表单（section 1）+ 摘要侧边栏 + 封面 MediaPicker
//   - 完整 form/hydrate/validate/buildPayload/submitForm 链路（佣金 fixed 模式）
//   - 嵌入 BundleStepsEditor（拆 step 2）渲染"2. 选择步骤"
// 设计：零 props，defineExpose({ open(row?) }) 让父级触发新建/编辑；
// emit('saved') 通知父级刷新列表。drawer 用 destroy-on-close，每次开都是
// 全新挂载，state 自然干净（无需手动 reset productSelectOptions / SKU map）。
...
// ============ Commission（fixed bundle 模式：抽屉 hydrate / buildPayload 用） ============
...
// ============ Group/option 纯函数（与 BundleStepsEditor 部分重复，因 validate
//             /buildPayload/summary panel 在抽屉这层使用；约 35 行，本地保留比抽
//             公共模块更贴合 home-sections 子组件风格） ============
...
const coverPreviewUrl = computed(() => {
  const reference = String(form.cover_image || form.cover_file_id || '').trim()
  if (!reference) return ''
  if (isCloudId(reference)) return imagePreviewCache[reference] || ''
  return reference
})
