...
exports.getBubbles = async (req, res) => {
    try {
        ...
        try {
            ...
            logger.warn('ACTIVITY_CTRL', 'getBubbles orders query failed', { error: dbErr.message });
        }
        ...
        try {
            ...
            logger.warn('ACTIVITY_CTRL', 'getBubbles lottery query failed', { error: lotErr.message });
        }
        ...
        logger.error('ACTIVITY_CTRL', 'getBubbles error', { error: err?.message || err });
...
exports.getFestivalConfig = async (req, res) => {
    try {
        ...
        logger.error('ACTIVITY_CTRL', '获取活动页配置失败', { error: error?.message || error });
        res.json({
            code: 0,
            data: {
                active: false,
...
exports.getGlobalUiConfig = async (req, res) => {
    try {
        ...
        logger.error('ACTIVITY_CTRL', '获取全局UI配置失败', { error: error?.message || error });
...
exports.getActivityLinksPublic = async (req, res) => {
    try {
        ...
        let raw = { banners: [], permanent: [], limited: [] };
        if (row?.config_value) {
            try { raw = { ...raw, ...JSON.parse(row.config_value) }; } catch (e) { logger.warn('ACTIVITY_CTRL', 'JSON.parse global_ui_config 失败', { error: e.message }); }
        }
        const data = sanitizeActivityLinksForPublic(raw);
        res.json({ code: 0, data });
    } catch (error) {
        logger.error('ACTIVITY_CTRL', '获取活动链接配置失败', { error: error?.message || error });
        res.json({
            code: 0,
            data: sanitizeActivityLinksForPublic({ banners: [], permanent: [], limited: [] })
        });
    }
};
...
exports.getLimitedSpotDetail = async (req, res) => {
    try {
        ...
        logger.error('ACTIVITY_CTRL', 'limited-spot detail', { error: error?.message || error });
