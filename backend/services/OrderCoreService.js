...
const _markOrderAsPaid = async (order, t) => {
    ...
    // 成长值：默认全场「实付订单金额」取整，与消费积分 floor(total_amount) 同口径（1 元 ≈ 1 成长值 × 后台倍数）；
    // 商品可在后台单独配置 product_growth_reward_{id} 覆盖为「每件固定成长基数」
    ...
    runAfterCommit(t, async () => {
        ...
        // N路径差价佣金：小n下单后，计提价差到大N的可提现余额（独立事务保原子性）
        try {
            const ROLES = require('../config/constants').ROLES;
            if (buyer.role_level === ROLES.N_MEMBER && buyer.n_leader_id) {
                ...
                if (nGap > 0) {
                    const { CommissionLog, sequelize: seq } = require('../models');
                    const nTx = await seq.transaction();
                    try {
        ...
        } catch (e) { console.error('[N路径差价] 计提失败:', e.message); }
    });
...
class OrderCoreService {
    static async createOrder(req) {
        ...
        try {
            ...
            items = items.map((it) => ({
                ...it,
                sku_id: normalizeSkuIdForFk(it.sku_id)
            }));
            ...
            // N路径：小n下单时额外获取大N的拿货价，用于计算差价佣金
            let nLeaderUser = null;
            if (roleLevel === constants.ROLES.N_MEMBER && user.n_leader_id) {
                nLeaderUser = await User.findByPk(user.n_leader_id, { transaction: t });
            }
                ...
                if (slash_no) {
                    ...
                    if (groupOrder.activity && groupOrder.activity.sku_id != null && groupOrder.activity.sku_id !== '') {
                        ...
                        if (!Number.isFinite(actSku) || reqSku !== actSku) {
                            await t.rollback();
                            throw new Error('拼团活动限定了规格，请在确认订单时选择与活动一致的「商品属性」');
                        }
                    }
